<template>
  ...
  <AppBaseList v-else :title="title" :routes="routes" :use-banner="true" :urlBanner="urlBanner">
    <template v-slot:actions>
      <AppButtonBack :to="{ name: 'surveys.userConvocations.registration' }" />

      <SaveAndExitStep :disabled="incompleteStep || (!incompleteStep && allFilesApproved)" :convocation-enrol-id="convocationEnrolId" :convocation-step-id="stepId" />
    </template>
    ...
    <template v-slot:content>
          ...
          <AppCard v-for="(detail, index) in step?.detail" class="card-hover" :key="index">
              ...
              <div class="d-flex justify-content-end">
                <span
                  :class="{
                    ...
                    'text-white bg-primary': documentStatus(detail) === 'Finalizado',
            ...
            <template #footer>
              <div v-if="!detail.enrolAttachment?.urlAttachment" class="d-flex justify-content-end">
                <div>
                  <AppButtonGroup>
                    ...
                    <AppButton :disabled="approvedOrInReviewStep && !incompleteStep" class="text-white" @click="clickInputFile(detail)" :label="'core.select'"></AppButton>
              ...
              <div v-if="detail.enrolAttachment?.urlAttachment" class="d-flex justify-content-start">
                <div>
                  <AppButton class="text-white" :label="'core.delete'" :disabled="detail.enrolAttachment && detail.enrolAttachment.isAccept === 1" @click="openConfirmDelete(detail)"></AppButton>
                </div>
              </div>
            </template>
          </AppCard>
      ...
      <AppModal v-model="modalDelete">
        <AppConfirmDeleteModal v-if="modalDelete" :entity="currentData?.enrolAttachment.title" @confirmDelete="confirmDelete()"></AppConfirmDeleteModal>
      </AppModal>
    </template>
  </AppBaseList>
</template>
