<template>
  ...
  <template v-else-if="!loading && activity && level">
    <ActivitiesTeacherForm :title="title" :subtitle="subtitle" :routes="routes" :activity="activity" :level="level" :competencies-by-period="competenciesByPeriod"></ActivitiesTeacherForm>
  </template>
</template>
...
export default defineComponent({
  name: 'UserCreate',
  components: { ActivitiesTeacherForm, AppLoading },
  setup() {
    const { t } = useI18n();
    ...
    onMounted(async () => {
      try {
        ...
        let activityId = parseInt(route.params.activityId.toString(), 10);

        const response = await getActivitiesRelatedDataFormService.run(courseId, activityId);
        ...
        activity.value = await getActivityByIdService.run(activityId);
        ...
        // Filling learningOutcomesIds from the form
        const learningOutcomesIds: any[] = [];
        response.learningOutcomes.forEach((elemt: any) => {
          if (elemt.checked) learningOutcomesIds.push(elemt.id);
        });
