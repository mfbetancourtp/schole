<template>
  <ActivitiesTeacherForm :title="title" :subtitle="subtitle" :routes="routes" :level="level" :competencies-by-period="competenciesByPeriod"></ActivitiesTeacherForm>
</template>
    ...
    onMounted(async () => {
      try {
        ...
        for (let i = 0; i < periods.length; i++) {
          ...
          const competences = await getListCompetencesService.run(curriculumSubjectId, periodId);
