<template>
  <AppBaseList :title="title" :subtitle="subtitle" isCard>
    <template v-slot:actions>
      <AppButton
        style="color: white"
        label="classroom.attendance.attendanceSummary.title"
        :to="{
          name: !isCourseExecution ? 'administration.homeTeacher.attendance.attendanceSummary' : 'training.executionOfCoursesAttendanceAttendanceSummary',
          params: { courseId },
        }"
      ></AppButton>
      ...
      <AppCheckPermission permission="classroom.attendance.create">
        <AppButton
          style="color: white"
          label="core.newFemale"
          :to="{
            name: !isCourseExecution ? 'administration.homeTeacher.attendance.create' : 'training.executionOfCoursesAttendanceCreate',
            params: { courseId },
          }"
        ></AppButton>
      </AppCheckPermission>
    ...
    <template v-slot:content>
      ...
      <template v-else>
        <section class="mb-3">
          <p class="mb-1">{{ t('classroom.attendance.filters.title') }}</p>
          ...
          <AppDatatableColumn label="core.table.actions" v-slot="{ row }" :position="'centered'" width="9rem">
            <div class="d-flex justify-content-center">
              <AppButtonGroup size="sm">
                <AppCheckPermission permission="classroom.attendance.edit">
                  <AppButtonEdit
                    :to="{
                      name: !isCourseExecution ? 'administration.homeTeacher.attendance.edit' : 'training.executionOfCoursesAttendanceEdit',
                      params: { courseId, sessionId: row.id },
                    }"
                  ></AppButtonEdit>
                </AppCheckPermission>

                <AppCheckPermission permission="classroom.attendance.delete">
...
import { DeleteAttendanceService } from '../services/deleteAttendance.service';
import { UpdateDatatableService } from '../../../shared/services/updateDatatable.service';
import { GetSessionsByCourseIdWithPaginationService } from '../services/getSessionsByCourseIdWithPagination.service';
...
export default defineComponent({
  ...
  setup() {
    ...
    const title = t('classroom.homeTeacher.attendance');
    const courseId = parseInt(route.params.courseId.toString(), 10);
    const service = new GetSessionsByCourseIdWithPaginationService();
    ...
    const params = computed(() => {
      ...
      if (date.dateSince) params.dateSince = date.dateSince;
      if (date.dateUntil) params.dateUntil = date.dateUntil;
    ...
    const confirmDelete = async () => {
      if (currentSessionId.value) {
        await deleteAttendanceService.run(currentSessionId.value);
