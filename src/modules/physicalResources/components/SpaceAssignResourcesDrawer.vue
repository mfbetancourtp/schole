<template>
  <AppDrawer :isVisible="isVisible" @update:isVisible="$emit('update:isVisible', $event)" size="xl">
    <!-- HEAD -->
    <template #head>
      <AppDrawerHeader variant="create" icon="clipboard-list" title="Asignar Recursos" :subtitle="space?.name ?? ''">
        <template #meta v-if="space?.hierarchicalPath">
          <span class="code-badge">{{ space.hierarchicalPath }}</span>
        </template>
      </AppDrawerHeader>
    </template>
    ...
    <template #body>
      <div class="tw-p-4">
        <!-- Search full width -->
        <div class="tw-mb-3">
          <input type="text" class="ext-search tw-w-full" placeholder="Buscar por nombre, tipo o estado..." v-model="searchQuery" />
        </div>

        <!-- Filters -->
        <div class="d-flex align-items-center gap-2 tw-mb-4">
          <select class="form-select tw-flex-1" v-model="filterType">
            <option value="">Todos los tipos</option>
            <option v-for="t in resourceCategories" :key="t" :value="t">
              {{ t }}
            </option>
          </select>
          <select class="form-select tw-flex-1" v-model="filterStatus">
                  ...
                  <tr v-for="resource in filteredAvailable" :key="resource.id" class="assign-table__row">
                    ...
                    <td>
                      <div class="d-flex flex-column gap-1">
                        <span v-for="code in resource.availableCodes" :key="code" class="code-tag">{{ code }}</span>
                      </div>
                    </td>
                    <td>
          ...
          <div class="d-flex flex-column gap-2">
            <div v-for="(assigned, idx) in assignedResources" :key="assigned.id" class="assigned-card" :class="{ 'assigned-card--editing': editingIdx === idx }">
              ...
              <div class="d-flex align-items-center flex-wrap gap-1">
                ...
                <span v-for="code in assigned.codes.slice(0, 3)" :key="code" class="code-tag">{{ code }}</span>
    ...
    <template #footer>
      <div class="d-flex align-items-center justify-content-between">
        ...
        <div class="d-flex gap-2">
          <AppButton outlined @click="$emit('update:isVisible', false)">Cancelar</AppButton>
          <AppButton :disabled="selectedIds.length === 0 && assignedResources.length === 0" @click="submit"> Asignar seleccionados </AppButton>
        </div>
      </div>
    </template>
  </AppDrawer>
...
export default defineComponent({
  name: 'SpaceAssignResourcesDrawer',
  ...
  props: {
    ...
    space: { type: Object as PropType<SpaceDto | null>, default: null },
  },
  ...
  setup(props, { emit }) {
    ...
    const filteredAvailable = computed(() => {
      ...
      if (filterType.value) result = result.filter((r) => r.category === filterType.value);
      return result;
    });
    ...
    const getAllCodesFor = (assigned: AssignedResource) => availableResources.value.find((r) => r.name === assigned.name)?.allCodes ?? assigned.codes;
    ...
    const saveEdit = (idx: number) => {
      ...
      editingIdx.value = null;
      editingCodes.value = [];
      toastService.show(`Unidades de "${name}" actualizadas`, 'success');
    };
    ...
    const submit = () => {
      const newAssigned = selectedIds.value.map((id) => {
        const r = availableResources.value.find((x) => x.id === id)!;
        return {
          id: r.id,
          name: r.name,
          category: r.category,
          quantity: 1,
          codes: r.allCodes.slice(0, 1),
        };
      });
      emit('save', {
...
.drawer-head-content__space {
  font-size: 0.85rem;
  font-weight: 500;
  color: rgba(255, 255, 255, 0.75);
}
.drawer-head-content__path-icon {
  font-size: 0.72rem;
  color: rgba(255, 255, 255, 0.6);
}
.drawer-head-content__path {
  font-size: 0.75rem;
  color: rgba(255, 255, 255, 0.65);
}

/* Search — same style as ResourceInventory ext-search */
.ext-search {
  display: block;
  width: 100%;
  ...
  border: var(--border-1) solid rgba(var(--color-gray-600-rgb), var(--opacity-color-60));
}
...
/* Remove Bootstrap's 2px black border-top on the first tbody row */
.assign-table tbody tr:first-child td {
  border-top: none;
}
