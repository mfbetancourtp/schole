<template>
  <AppBaseList :title="title">
    <template #actions>
      <AppButton style="color: white" v-if="selectedRecord" type="button" outlined @click="closeDetail">Volver al listado</AppButton>
    </template>
        ...
        <template v-if="!selectedRecord">
          <div class="sectionHead">
            <div class="contentIconTitle">
              <div class="sectionHead__icon">
                <AppIcon icon="file-medical" />
              </div>
              <div>
                <h2 class="sectionHead__title">Validación de Hoja de Vida Docente</h2>
                <p class="sectionHead__subtitle">Consulta el consolidado de adjuntos por docente y prioriza las validaciones pendientes.</p>
              </div>
            </div>
          </div>
          <AppDatatable
            name="attachment-validation"
            :service="getAttachmentValidationWithPaginationService"
            :params="tableParams"
          ...
          >
            <template #additionalContent>
              <div class="attachmentValidationPage__tableHead">
                <div class="attachmentValidationPage__tableTitle">
                  <div class="attachmentValidationPage__tableTitleLabel">
                    <AppIcon icon="tasks" class="me-2" />
                    Solicitudes de validación
                  </div>

                  <AppBadge :label="`${tableTotal} registros`" variant="ghost" />
                </div>

                <p class="attachmentValidationPage__tableHint">La búsqueda general queda disponible en la tabla y el filtro avanzado por estado se aplica desde el panel lateral.</p>
              </div>
            </template>
            ...
            <template #filters>
              <AppFilter>
                <AppFormField label="Estado">
                  <select v-model="pendingStatus" class="form-control">
            ...
            <AppDatatableColumn field="fullName" label="Docente" v-slot="{ row }">
              <div class="teacherCell">
                <div class="teacherAvatar">
                  <img v-if="row.teacherPhoto" :src="row.teacherPhoto" :alt="row.teacherName" />
                  <span v-else>{{ getInitials(row.teacherName) }}</span>
                </div>

                <div>
                  <div class="teacherName">{{ row.teacherName }}</div>
                  <div class="teacherMeta">{{ row.teacherPosition }}</div>
                </div>
              </div>
            </AppDatatableColumn>

            <AppDatatableColumn field="totalDocuments" label="Documentos cargados" position="centered" v-slot="{ row }">
              <span class="documentsCount">
                <strong>{{ row.totalDocuments }}</strong> soportes
              </span>
            </AppDatatableColumn>

            <AppDatatableColumn field="lastUpdate" label="Última actualización" v-slot="{ row }">
              <span>{{ formatDateTime(row.lastUpdate) }}</span>
            </AppDatatableColumn>

            <AppDatatableColumn field="overallStatus" label="Estado" position="centered" v-slot="{ row }">
              <AppBadge :label="row.overallStatus" :variant="getStatusVariant(row.status)" />
            </AppDatatableColumn>

            <AppDatatableColumn label="Acciones" position="centered" v-slot="{ row }">
              <div class="actionsCell">
                <AppButton type="button" outlined @click="selectRecord(row)">
        ...
        <template v-else>
          <div class="detailPage">
            ...
            <template v-else-if="cvDetail">
                  ...
                  <div class="detailHero__content">
                    <div class="detailHero__heading">
                      ...
                      <div class="detailHero__badges">
                        <AppBadge :label="selectedRecord.overallStatus" :variant="getStatusVariant(selectedRecord.status)" />
                      ...
                      <template #body>
                        <div v-if="activeDocument" class="documentPreview">
                          ...
                          <div v-if="hasActiveDocumentViewer" class="documentViewer documentViewer--loaded">
                            <img
                              v-if="activeDocumentViewer.kind === 'image'"
                              :key="activeDocumentViewer.src"
                              :src="activeDocumentViewer.src"
                              :alt="activeDocument.title"
                              class="documentViewer__image"
                            />
              ...
              <AppCard class="reviewSummaryCard">
                <template #title>Resumen por sección</template>
                <template #body>
                  <div class="sectionSummary">
                    <div v-for="section in visibleSections" :key="`summary-${section.key}`" class="sectionSummary__row">
                      <div class="sectionSummary__left">
                        <AppIcon :icon="section.icon" />
                        <span>{{ section.label }}</span>
                      </div>

                      <AppBadge :label="String(section.items.length)" variant="ghost" />
                    </div>
                  </div>
                </template>
              </AppCard>
...
import type {
  ...
  AttachmentValidationStatusFilter,
  AttachmentValidationSummaryDto,
...
import { GetAttachmentValidationSummaryService } from '../services/getAttachmentValidationSummary.service';
import { GetAttachmentValidationWithPaginationService } from '../services/getAttachmentValidationWithPagination.service';
...
const getAttachmentValidationSummaryService = new GetAttachmentValidationSummaryService();
const getAttachmentValidationWithPaginationService = new GetAttachmentValidationWithPaginationService();
...
export default defineComponent({
  ...
  setup() {
    ...
    const selectedRecord = ref<AttachmentValidationRecordDto | null>(null);
    const summary = ref<AttachmentValidationSummaryDto>({
      pending: 0,
      approved: 0,
      rejected: 0,
    });
    ...
    const appliedStatus = ref<AttachmentValidationStatusFilter>('all');
    const pendingStatus = ref<AttachmentValidationStatusFilter>('all');
    ...
    const isReviewRoute = computed(() => route.name === attachmentValidationRouteNames.review);

    const totalRecords = computed(() => summary.value.pending + summary.value.approved + summary.value.rejected);

    const tableTotal = computed(() => {
      const response = checkStore.isCheckData.dataTable;
      return Number(response?.total ?? response?.pagination?.total ?? totalRecords.value);
    });

    const tableParams = computed(() => ({
      ...(appliedStatus.value !== 'all' ? { status: appliedStatus.value } : {}),
    }));
    ...
    const detailFullName = computed(() => {
      if (!cvDetail.value) return selectedRecord.value?.teacherName ?? '';
    ...
    const canUseReviewWorkspace = () => {
      ...
      return window.innerWidth > 1200 && Boolean(selectedRecord.value);
    };
    ...
    const loadSummary = async () => {
      summary.value = await getAttachmentValidationSummaryService.run();
    };
    ...
    const loadCvDetail = async (sectionKey?: AttachmentValidationCvSectionKey) => {
      ...
      try {
        cvDetail.value = await getAttachmentValidationPeopleDetailService.run({
          peopleId: selectedRecord.value.peopleId,
    ...
    const applyPendingFilters = () => {
      appliedStatus.value = pendingStatus.value;
    };

    const syncPendingFilters = () => {
      pendingStatus.value = appliedStatus.value;
    };
    ...
    const selectRecord = async (record: AttachmentValidationRecordDto) => {
      await router.push({
        name: attachmentValidationRouteNames.review,
        params: { peopleId: record.peopleId },
        query: {
          teacherName: record.teacherName,
          teacherPosition: record.teacherPosition,
          totalDocuments: String(record.totalDocuments),
    ...
    const changeSection = async (sectionKey: AttachmentValidationCvSectionKey | '') => {
      if (!selectedRecord.value || !isReviewRoute.value) return;
      ...
      await router.replace({
        name: attachmentValidationRouteNames.review,
        params: { peopleId: selectedRecord.value.peopleId },
        query: nextQuery,
      });
    };
    ...
    watch(
      ...
      async () => {
        ...
        const shouldReload = selectedRecord.value?.peopleId !== routeRecord.peopleId || selectedSectionKey.value !== nextSectionKey || !cvDetail.value;

        selectedRecord.value = routeRecord;
    ...
    return {
      ...
      previewModalVisible,
      isSubmittingStatus,
      getAttachmentValidationWithPaginationService,
      getInitials,
      getStatusVariant,
