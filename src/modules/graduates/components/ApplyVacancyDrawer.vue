<template>
  <AppDrawer :isVisible="isVisible" @update:isVisible="$emit('update:isVisible', $event)" size="md">
    <template #head>
      <AppDrawerHeader variant="create" icon="paper-plane" title="Aplicar a vacante" :subtitle="vacancy ? `${vacancy.title} · ${vacancy.company}` : ''" />
    </template>
      ...
      <div class="drawer-section" v-if="isVisible">
        ...
        <div v-if="vacancy" class="apply-vacancy-info">
          <div class="apply-vacancy-logo">
            ...
            <span v-else class="apply-logo-placeholder">{{ vacancy.company?.[0] ?? '?' }}</span>
          </div>
          <div>
        ...
        <div class="row g-3">
          <!-- CV selection -->
          <div class="col-12">
            <AppFormField label="Selecciona tu CV *">
          ...
          <!-- Cover letter -->
          <div class="col-12">
            <AppFormField label="Carta de presentación">
              <textarea
                class="form-control"
          ...
          <!-- Portfolio -->
          <div class="col-12">
            <AppFormField label="Portafolio o enlace relevante">
    ...
    <template #footer>
      <div class="d-flex align-items-center justify-content-between w-100">
        ...
        <div class="d-flex gap-2">
          ...
          <AppButton variant="primary" nativeType="button" :loading="saving" @click="save"> <AppIcon icon="paper-plane" /> Enviar postulación </AppButton>
        </div>
      </div>
    </template>
  </AppDrawer>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const errors = reactive({ cvFileId: '', consent: '' });
