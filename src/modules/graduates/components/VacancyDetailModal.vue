<template>
  <AppFormModal title="Detalle de la vacante">
    <template #content>
      <div v-if="vacancy" class="vd-wrap">
        <!-- Header: logo + título + empresa -->
        <div class="vd-header">
          <div class="vd-logo">
            <img v-if="vacancy.companyLogo" :src="vacancy.companyLogo" :alt="vacancy.company" />
            <span v-else class="vd-logo-letter">{{ vacancy.company.charAt(0) }}</span>
          </div>
          <div>
            <h5 class="vd-title">{{ vacancy.title }}</h5>
            <p class="vd-company">{{ vacancy.company }}</p>
          </div>
        </div>
        ...
        <div class="vd-meta-grid">
          <div class="vd-meta-item">
            ...
            <div>
              ...
              <p class="vd-meta-value">{{ vacancy.city }}, {{ vacancy.country }}</p>
            </div>
          </div>
          <div v-if="vacancy.salaryMin" class="vd-meta-item">
        ...
        <!-- Applicants count -->
        <div class="vd-applicants">
          <AppIcon icon="users" />
          <span>{{ vacancy.applicantsCount }} personas ya se han postulado</span>
        </div>
      </div>
    </template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // Split requirements and benefits by line breaks or ". " into bullet items
