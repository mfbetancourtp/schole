...
export class ImportFinancingSystemsTemplateService {
  async run(data: any, params: any = {}): Promise<void> {
    const newData = jsonToFormDataService.run(data);

    return await axios.post(`${services.budget}/admin/template-financing-sources-import`, newData, { params }).then((response) => response.data);
  }
}
