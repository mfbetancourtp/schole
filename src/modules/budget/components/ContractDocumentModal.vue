<template>
  <form @submit.prevent="save">
    <AppFormModal :title="title + (v$.form.name.$model === undefined ? '' : v$.form.name.$model)">
      <template v-slot:content>
        <AppLoading v-if="loading"></AppLoading>
        <template v-else>
          <div class="form-row">
            <AppFormField :form-control="v$.form.name">
              <label for="name">Nombre</label>
              <input type="text" class="form-control" v-model="v$.form.name.$model" />
            </AppFormField>
          </div>
          <div class="form-row">
            <AppFormField :form-control="v$.form.description">
              <label for="naspareKgme">Descripción</label>
              <input type="text" class="form-control" v-model="v$.form.description.$model" />
            </AppFormField>
          </div>
          <div class="form-row">
            <AppFormField :form-control="v$.form.categoryId" label="Categoría">
              <AppAutocomplete v-model="v$.form.categoryId.$model" :options="categories" label="value" :clearable="false" :reduce="(d:any)=>d.label"></AppAutocomplete>
            </AppFormField>
          </div>
        </template>
      </template>
      <template v-slot:actions>
        <AppButton :label="'core.save'" :nativeType="'submit'"></AppButton>
      </template>
    </AppFormModal>
  </form>
</template>

<script lang="ts">
import { defineComponent, onMounted, PropType, reactive, ref, toRefs, toRef } from 'vue';
import { required } from '../../../shared/plugins/vuelidate.plugin';
import useVuelidate from '@vuelidate/core';

import AppFormModal from '../../../shared/components/Modal/AppFormModal.vue';
import AppFormField from '../../../shared/components/Forms/AppFormField.vue';
import AppButton from '../../../shared/components/Buttons/AppButton.vue';
import AppAutocomplete from '../../../shared/components/Forms/AppAutocomplete.vue';
import AppLoading from '../../../shared/components/AppLoading.vue';

import { CreateOrUpdateDocumentsBudgetsService } from '../services/createOrUpdateDocumentsBudgets.service';
import { GetDocumentsBudgetsCategoriesService } from '../services/getDocumentsBudgetsCategories.service';
const getDocumentsBudgetsCategoriesService = new GetDocumentsBudgetsCategoriesService();

export default defineComponent({
  name: 'ContractDocumentModal',
  components: {
    AppFormModal,
    AppFormField,
    AppButton,
    AppAutocomplete,
    AppLoading,
  },
  props: {
    data: {
      type: Object as PropType<any | null>,
    },
    categoryId: {
      type: Number as PropType<number | null>,
      required: true,
    },
  },
  emits: ['close'],
  setup(props, { emit }) {
    const createOrUpdateDocumentsBudgetsService = new CreateOrUpdateDocumentsBudgetsService();
    const { data } = toRefs(props);
    const categoryId = ref();

    let title = 'Agregar documento ';
    if (props.data?.id) {
      title = 'Editar  documento ';
    } else {
      categoryId.value = toRef(props, 'categoryId');
    }
    const loading = ref(true);
    const categories = ref([]);
    const form: any = reactive({
      name: data.value?.name,
      description: data.value?.description,
      categoryId: data.value?.categoryId ?? categoryId.value ?? null,
    });
    const v$ = useVuelidate(
      {
        form: {
          name: { required },
          description: { required },
          categoryId: { required },
        },
      },
      { form }
    );
    onMounted(async () => {
      try {
        categories.value = await getDocumentsBudgetsCategoriesService.run();
      } catch (error) {
        console.log('This is error', error);
      }

      loading.value = false;
    });
    const save = async () => {
      const formIsValid = await v$.value.$validate();
      if (!formIsValid) return;
      loading.value = true;
      try {
        await createOrUpdateDocumentsBudgetsService.run(form, data.value?.id);
        emit('close');
      } catch (error) {
        console.log('This is error', error);
      }
      loading.value = false;
    };
    return {
      title,
      loading,
      v$,
      save,
      categories,
    };
  },
});
</script>
