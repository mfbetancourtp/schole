...
import { CreateOrUpdateNaturePaymentService } from '../services/createOrUpdateNaturePayment.service';

export default defineComponent({
  name: 'NaturePaymentModal',
  components: {
    AppFormModal,
    AppFormField,
    AppButton,
  ...
  emits: ['close'],
  setup(props, { emit }) {
    const createOrUpdateNaturePaymentService = new CreateOrUpdateNaturePaymentService();
    const { data } = toRefs(props);
    let title = 'Agregar naturaleza de pago ';
    if (props.data?.id) {
      title = 'Editar naturaleza de pago ';
    }
    ...
    const save = async () => {
      ...
      loading.value = true;
      try {
        await createOrUpdateNaturePaymentService.run(form, data.value?.id);
        loading.value = false;
        emit('close');
      } catch (error) {
