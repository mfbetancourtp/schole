...
import { CreateOrUpdateBanksService } from '../services/createOrUpdateBanks.service';
...
export default defineComponent({
  ...
  emits: ["close"],
  setup(props, { emit }) {
    const createOrUpdateBanksService = new CreateOrUpdateBanksService();
    const data = props.data;
    let title = 'Agregar banco ';
    if (props.data?.id) {
      title = "Editar banco ";
    };
    ...
    const save = async () => {
      ...
      loading.value = true;
      try {
        await createOrUpdateBanksService.run(form, data?.id);
        loading.value = false;
        emit("close");
      } catch (error) {
