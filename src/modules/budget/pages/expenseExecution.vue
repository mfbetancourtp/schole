            ...
            <template #footer>
              <div class="buttons-card">
                <div>
                  ...
                  <AppButton icon="plus" v-tooltip="'core.new'" @click="redirectCreateView(financingSystem.id)"></AppButton>
                </div>
                <div class="button-left">
                  <AppButton
                    icon="file-invoice-dollar"
                    v-tooltip="'core.view cdp'"
                    @click="redirectViewCdp(financingSystem.executionOfExpenseId)"
                    :disabled="!financingSystem.executionOfExpenseId"
                  ></AppButton>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getData = async () => {
      try {
        const params = { executionExpense: true };
        financingSystems.value = await getFinancingClassificationSystemService.run(params);
      } catch (e) {
        console.log('error', e);
      }
    };
    ...
    const redirectCreateView = async (systemId: number) => {
      await router.push({
        name: 'budget.expenseExecutionCreate',
        params: {
          accountingSystemId: systemId,
        },
      });
    };
    const redirectViewCdp = async (executionOfExpenseId: number) => {
      await router.push({
        name: 'budget.CdpsBudget',
        params: {
