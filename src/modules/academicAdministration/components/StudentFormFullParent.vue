<template>
  ...
  <template v-else>
    ...
    <template v-else>
      <div class="title-form">{{ t(title) }}</div>
      <div class="d-flex align-items-center justify-content-end form-row">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    //const currentParent = ref<ParentDetailDto | null>(null);
    //const matriculateId = parseInt(route.params.matriculateId as string);
    const matriculateId = computed(() => props.matriculateId);
    ...
    const form = reactive({
      ...
      isEconomicManager: props.parent?.isEconomicManager || false,
      isEmergencyContact: props.parent?.isEmergencyContact || false,
      isACollaborator: props.parent?.people?.isACollaborator || false,
    ...
    onMounted(async () => {
      ...
      try {
        ...
        if (props.parent?.id) validateUser.value = true;
      } catch (e) {
        console.log('err', e);
      }

      loading.value = false;
      console.log('Loading complete:', { validateUser: validateUser.value });
    });
    const resetForm = () => {
      //validate identification reset
      formIdentification.identification = '';
      // form parent reset
      form.relationshipId = null;
      form.isEconomicManager = false;
      form.isEmergencyContact = false;
      form.isACollaborator = false;

      form.basiData.id = null;
      form.basiData.identificationTypeId = null;
      form.basiData.identification = null;
      form.basiData.names = null;
      form.basiData.lastnames = null;
      form.basiData.email = null;
      form.basiData.mobile = null;
      form.basiData.alternateEmail = null;
      form.basiData.homePhone = null;
      form.basiData.workPhone = null;

      form.persData.genderId = null;
      form.persData.birthCountryId = null;
      form.persData.birthRegionId = null;
      form.persData.birthMunicipalityId = null;
      form.persData.birthDate = null;
      form.persData.nationality = null;
      form.persData.direction = null;
      form.persData.zone = null;
      form.persData.religion = null;
      form.persData.primaryLanguage = null;

      form.emplData.companyName = null;
      form.emplData.companyPosition = null;
      form.emplData.companyAreaId = null;
    };

    watch(
      () => props.parent,
      (newParent) => {
        resetForm(); //reset form anterior si no se guardo.
        validateUser.value = !!props.parent?.id; //validateUser valida si aparece el apartado de validar identificacion

        if (newParent) {
          // Actualiza el formulario cuando `parent` cambie
          form.relationshipId = newParent.relationshipId ?? null;
          form.isEconomicManager = !!newParent.isEconomicManager;
          form.isEmergencyContact = !!newParent.isEmergencyContact;
          form.isACollaborator = !!newParent.people?.isACollaborator;

          form.basiData.id = newParent.people?.id ?? null;
          form.basiData.identificationTypeId = newParent.people?.identificationTypeId ?? null;
          form.basiData.identification = newParent.people?.identification ?? null;
          form.basiData.names = newParent.people?.names ?? null;
          form.basiData.lastnames = newParent.people?.lastnames ?? null;
          form.basiData.email = newParent.people?.email ?? null;
          form.basiData.mobile = newParent.people?.mobile ?? null;
          form.basiData.alternateEmail = newParent.people?.alternateEmail ?? null;
          form.basiData.homePhone = newParent.people?.homePhone ?? null;
          form.basiData.workPhone = newParent.people?.workPhone ?? null;

          form.persData.genderId = newParent.people?.genderId ?? null;
          form.persData.birthCountryId = newParent.people?.birthMunicipality?.region?.country?.id ?? null;
          form.persData.birthRegionId = newParent.people?.birthMunicipality?.region?.id ?? null;
          form.persData.birthMunicipalityId = newParent.people?.birthMunicipality?.id ?? null;
          form.persData.birthDate = newParent.people?.birthDate ?? null;
          form.persData.nationality = newParent.people?.nationality ?? null;
          form.persData.direction = newParent.people?.direction ?? null;
          form.persData.zone = newParent.people?.zone ?? null;
          form.persData.religion = newParent.people?.religion ?? null;
          form.persData.primaryLanguage = newParent.people?.primaryLanguage ?? null;

          form.emplData.companyName = newParent.people?.companyName ?? null;
          form.emplData.companyPosition = newParent.people?.companyPosition ?? null;
          form.emplData.companyAreaId = newParent.people?.companyAreaId ?? null;
        }
      },
      { immediate: true }
    );
    ...
    const save = async () => {
      ...
      try {
        ...
        console.log('test', matriculateParent);
        const dataSend = { people, matriculateParent };
        await createOrUpdateMatriculateParentsService.run(dataSend);
        resetForm();
