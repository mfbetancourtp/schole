...
export class ImportNotesFromTemplateService {
  async run(data: any): Promise<void> {
    let newData = jsonToFormDataService.run(data);

    return await axios.post(`${services.academic}/academic-administration/import-matriculate-subject-periods`, newData)
