<template>
  ...
  <template v-else>
        ...
        <div class="academy-info-nav">
          <div class="academy-info-item">
            <div class="container-academic-periods">
              <AppAutocomplete v-model="academicPeriodId" label="academicPeriodName" :clearable="false" :options="data.academicPeriodEnrollments" :reduce="(item: any) => item.academicPeriodId" />
            </div>
      ...
      <template v-if="!loadingInfo">
        ...
        <AppCard>
          <template #body>
            ...
            <section>
              <AppCheckPermission permission="disciplinaryControl.disciplinaryOffenses.create" :hide="true">
                    ...
                    <div class="offense-actions d-flex flex-row-reverse">
                      <AppButtonGroup size="lg">
                        <AppCheckPermission permission="disciplinaryControl.disciplinaryOffenses.sendMail.list">
                          <AppButton variant="primary" icon="envelope" outlined v-tooltip="offense.isSentParents ? 'Volver a enviar email' : 'Enviar email'" @click="openSendMailModalModal(offense)" />
                        </AppCheckPermission>
    ...
    <div v-if="currentSectionForm === 'studentWellness'">
      <ObseByUserIdMedicalRecords :userId="Number(userId)" :permission="'disciplinaryControl.disciplinaryOffenses.medicalRecords'" />
    </div>
  </template>
...
import ObseByUserIdMedicalRecords from './ObseByUserIdMedicalRecords.vue';
import DisciplinaryOffensesFormDrawer from './DisciplinaryOffensesFormDrawer.vue';
import ConfirmSendMailParentsDisciplinaryOffenses from './ConfirmSendMailParentsDisciplinaryOffenses.vue';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    onMounted(async () => {
      ...
      data.value = await getStudentForObserverService.run(props.userId.toString());
      if (data.value.academicPeriodEnrollments.length > 0) {
        academicPeriodId.value = data.value.academicPeriodEnrollments[0].academicPeriodId;
        academicPeriodEnrollment.value = data.value.academicPeriodEnrollments[0];
      }
    ...
    watch(
      ...
      async (newUserId, oldUserId) => {
        if (newUserId !== oldUserId) {
          ...
          data.value = await getStudentForObserverService.run(newUserId.toString());
          ...
          if (data.value?.academicPeriodEnrollments?.length > 0) {
            ...
            academicPeriodEnrollment.value = null;
            offensesSummary.value = null;
          }
    ...
    const getOffensesSummary = async (academicPeriodId: number) => {
      ...
      offensesSummary.value = await getStudentOffensesSummaryService.run(props.userId.toString(), { academicPeriodId });
