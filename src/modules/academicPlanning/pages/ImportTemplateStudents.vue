<template>
  <AppBaseList :title="title" isCard>
    ...
    <template #content>
      ...
      <template v-else>
        <div>
          <DownloadAndUploadTemplate :keyService="'student'"></DownloadAndUploadTemplate>
        </div>
      </template>
    </template>
  </AppBaseList>
...
export default defineComponent({
  ...
  setup() {
    const { t } = useI18n();
    const title = 'Importar datos desde una plantilla de estudiantes';
    const routes: any[] = [{ name: title }];
    useMeta({ title: 'Importar plantilla estudiantes' });
