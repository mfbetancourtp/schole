<template>
  <AppBaseList :title="title" isCard>
    ...
    <template #content>
      ...
      <template v-else>
        <div>
          <DownloadAndUploadTemplate :keyService="'subjectAssignments'"></DownloadAndUploadTemplate>
        </div>
      </template>
    </template>
  </AppBaseList>
...
export default defineComponent({
  ...
  setup() {
    const { t } = useI18n();
    const title = 'Importar datos desde una plantilla de carga académica';
    const routes: any[] = [{ name: title }];
    useMeta({ title: 'Importar plantilla carga académica' });
