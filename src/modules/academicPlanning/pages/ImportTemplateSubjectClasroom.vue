<template>
  <AppBaseList :title="title" isCard>
    ...
    <template #content>
      ...
      <template v-else>
        <div>
          <DownloadAndUploadTemplate :keyService="'subjectClasroom'"></DownloadAndUploadTemplate>
        </div>
      </template>
    </template>
  </AppBaseList>
...
export default defineComponent({
  name: 'ImportTemplateSubjectAssignments',
  components: {
  ...
  setup() {
    const { t } = useI18n();
    const title = 'Importar datos desde una plantilla de horario de clases';
    const routes: any[] = [{ name: title }];
    useMeta({ title: 'Importar plantilla horario de clases' });
