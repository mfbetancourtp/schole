<template>
  <BaseLayoutDevelopmentMaps :title="`Reporte General de Avances ${profileCode !== 'student' ? 'por Colaborador' : ''}`">
    <template #actions>
      <AppButtonBack
    ...
    <template #content>
      ...
      <template v-else>
        ...
        <div class="Table__content">
          ...
          <table v-else class="Content__dataTable">
            <thead>
              <tr>
                <th>DKC</th>
                <th>Nombre</th>
                ...
                <th>Area</th>
                <th>Puesto</th>
                <th class="text-center"># Cursos Oblig.</th>
                <th class="text-center"># Cursos Opt.</th>
                <th class="text-center"># Cursos Oblig. Mapa</th>
            ...
            <tbody>
              <tr v-for="(report, index) in dataReport" :key="index">
                ...
                <td>{{ report.organizationalUnitName }}</td>
                <td>
                  {{ report.organizationalJobName ? deleteDuplicateValues(report.organizationalJobName) : '---' }}
                </td>
                <td class="text-center">{{ report.requiredCoursesNumber }}</td>
                <td class="text-center">{{ report.optionalCoursesNumber }}</td>
                <td class="text-center">
                  {{ report.requiredCoursesApproved }}
                </td>
                <td class="text-center">{{ report.totalPercentageOfCompletation.toFixed(2) }}%</td>
              </tr>
            </tbody>
          </table>
        </div>
...
import { GetDataGeneralReportService } from '../../services/organizational/getDataGeneralReport.service';
...
const getDataGeneralReportService = new GetDataGeneralReportService();
  ...
  setup() {
    ...
    const generateReport = async () => {
      try {
        ...
        const params = {
          employeeId: profileCode === 'student' ? employeeId.value : null,
          organizationalJobId: organizationalJobId.value,
          organizationalUnitId: organizationalUnitId.value,
        };

        dataReport.value = await getDataGeneralReportService.run(params);
      } catch (err) {
        console.log('err', err);
    ...
    const deleteDuplicateValues = (value: string) => {
