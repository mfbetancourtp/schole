                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr v-for="data, index in summaryBeforeExporting" :key="index">
                                    ...
                                    <td>{{ data.code ?? '--' }}</td>
                                    <td>{{ data.date }}</td>
                                    <td>{{ data.clientInvoice }}</td>
                                    <td>{{ data.paymentMethod }}</td>
                                    <td>{{ data.value }}</td>
                                    <td>{{ data.codeInvoice }}</td>
                                </tr>
                            </tbody>
                        </table>
...
import { ExportPaymentsService } from "../services/exportPayments.service";
import { GenerateSummaryBeforeExportingPaymentsService } from "../services/generateSummaryBeforeExportingPayments.service";
import { GetTemplatesEnabledForExportingPaymentsService } from "../services/getTemplatesEnabledForExportingPayments.service";
import { useHeaderStore } from "../../../stores/header.store";

const getTemplatesEnabledForExportingPaymentsService = new GetTemplatesEnabledForExportingPaymentsService();
const generateSummaryBeforeExportingPaymentsService = new GenerateSummaryBeforeExportingPaymentsService();
const exportPaymentsService = new ExportPaymentsService();
...
export default defineComponent({
    ...
    setup() {
      ...
      headerStorage.moduleSubItem = { name: title, url: '' };
                templatesEnabled.value = await getTemplatesEnabledForExportingPaymentsService.run();
        ...
        const search = async () => {
            ...
            try {
                summaryBeforeExporting.value = await generateSummaryBeforeExportingPaymentsService.run(form);
            } catch (e) {}
        ...
        const exportCollections = async () => {
            ...
            try {
                urlFile.value = await exportPaymentsService.run(form);
            } catch (e) {}
