...
<template>
  <div class="form-row">
    <AppFormField :form-control="v$.form.academicPeriodId" label="core.generalTerms.academic.academicPeriod.singular">
      <AppAutocomplete input-id="academicPeriodId" :options="academicPeriods" v-model="v$.form.academicPeriodId.$model"></AppAutocomplete>
    </AppFormField>

    <AppFormField :form-control="v$.form.levelId" label="core.generalTerms.academic.level.singular">
      <AppAutocomplete input-id="levelId" :options="levels" v-model="v$.form.levelId.$model" :disabled="!v$.form.academicPeriodId.$model"></AppAutocomplete>
    </AppFormField>
  </div>

  <div class="form-row">
    <AppFormField :form-control="v$.form.degreeId" label="core.generalTerms.academic.degree.singular">
      <AppAutocomplete input-id="degreeId" :options="degrees" v-model="v$.form.degreeId.$model" :disabled="!v$.form.levelId.$model"></AppAutocomplete>
    </AppFormField>

    <AppFormField :form-control="v$.form.groupId" label="core.generalTerms.academic.group.singular">
      <AppAutocomplete input-id="groupId" :options="groups" v-model="v$.form.groupId.$model" :disabled="!v$.form.degreeId.$model"></AppAutocomplete>
    </AppFormField>
  </div>
</template>
...
export default defineComponent({
  ...
  emits: ['selectedLevel'],
  // emit('selectedLevel', student.people.userId);
    ...
    watch(
      ...
      async (value) => {
        if (value) {
          ...
          emit(
            'selectedLevel',
            levels.value.find((level) => level.id === value)
          );
        } else {
          degrees.value = [];
        }
