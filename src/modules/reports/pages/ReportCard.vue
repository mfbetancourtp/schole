  ...
  <BaseReport v-else :title="title" :v$="v$" :routes="routes" :service="service" :params="params" :show-btn-generate="false">
    ...
    <div v-if="showListStudents" class="mt-2 table-responsive">
      <AppDatatable :service="service" :params="params" :name="name">
        ...
        <AppDatatableColumn :label="'Promedio'" field="average" :sortable="true" v-slot="{ row }">
          {{ row.finalPeriod }}
        </AppDatatableColumn>

        <AppDatatableColumn class="position" :label="'Puesto'" field="position" :sortable="true" v-slot="{ row }">
          <div class="d-flex gap-1">
            ...
            <div class="button-position">
              <AppButtonEdit @click="openFormModalEditPositionSubjectPeriods(row)"> </AppButtonEdit>
            </div>
          </div>
        </AppDatatableColumn>
