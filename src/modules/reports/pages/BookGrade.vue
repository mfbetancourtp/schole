<template>
  ...
  <BaseReport v-else :title="title" :v$="v$" :routes="routes" :service="service" :params="params" :show-btn-generate="false">
    ...
    <div class="responsive-buttons">
      ...
      <AppButton :label="'reports.filters.generateAll'" :disabled="!showListStudents" class="buttons" :native-type="'button'" @click="generateAll()" variant="primary"></AppButton>

      <AppButton :disabled="!showListStudents" :label="'Descargar todos'" class="buttons" :native-type="'button'" @click="downloadAll()"></AppButton>
    </div>
    ...
    <div v-if="showListStudents" class="mt-2 table-responsive">
      ...
      <AppModal v-model="sendMailModal">
        <ConfirmSendMailGradeBook v-if="sendMailModal" :filters="filtersAux.value" @close="closeSendMailModalModal"> </ConfirmSendMailGradeBook>
      </AppModal>
    </div>
  </BaseReport>
</template>
...
import ConfirmSendMailGradeBook from '../components/ConfirmSendMailGradeBook.vue';
...
import { GenerateBookGradePerStudentService } from '../services/reports/generateBookGradePerStudent.service';
import { DownloadBookGradeService } from '../services/reports/downloadBookGrade.service';
import { useHeaderStore } from '../../../stores/header.store';

const generateBookGradePerStudentService = new GenerateBookGradePerStudentService();
...
export default defineComponent({
  ...
  setup() {
    ...
    const title = 'reports.bookGrade';
    const permission = 'reports.control.bookGrade';
    ...
    const generateIndividualReport = async (data: any) => {
      ...
      try {
        const response = await generateBookGradePerStudentService.run({
          ...getFilterReports(),
          userId: data.userId,
