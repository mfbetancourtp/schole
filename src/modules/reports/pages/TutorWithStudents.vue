<template>
  ...
  >
    ...
    <!-- Botones para abrir cada Drawer -->
    <div class="form-row gap-8 mt-12">
      <AppButton @click="drawerStudentOpen = true">
        Seleccionar campos de información (Estudiante)
      </AppButton>
      <AppButton outlined @click="drawerTutorOpen = true">
        Seleccionar campos de información (Tutor)
      </AppButton>
    </div>

    <!-- Resumen de selección (chips) con altura fija -->
    <div
    ...
    >
      ...
      <div class="summary-block" v-if="studentSectionsLabels.length || studentOptionsLabels.length">
        <div class="summary-head">
          ...
          <span class="summary-count">
            {{ studentSectionsLabels.length + studentOptionsLabels.length }}
          </span>
        </div>
      ...
      <div class="summary-block" v-if="tutorSectionsLabels.length || tutorOptionsLabels.length">
        <div class="summary-head">
          ...
          <span class="summary-count">
            {{ tutorSectionsLabels.length + tutorOptionsLabels.length }}
          </span>
        </div>
...
import { GenerateReportTutorWithStudentsListService } from '../services/reports/generateReportTutorWithStudentsList.service';
...
export default defineComponent({
  ...
  setup() {
    const title = 'reports.tutorWithStudentsList';
    const permission = 'reports.listings.tutorWithStudentsList';
    ...
    // Opciones (preview) para cada tipo
    ...
    // ————— Preview desde cada Drawer (NO compromete showFields*) —————
    ...
    watch(
      ...
      async (newMap) => {
        try {
          const sectionSelect = Object.keys(newMap || {}).filter((k) => newMap[k]);
          ...
          // Purga de opciones comprometidas que ya no existan
          ...
          console.log('Error al recalcular opciones estudiante:', e);
          showResponseOptionsStudent.value = {};
          // @ts-ignore
    ...
    watch(
      ...
      async (newMap) => {
        try {
          ...
          console.log('Error al recalcular opciones tutor:', e);
          showResponseOptionsTutor.value = {};
          // @ts-ignore
