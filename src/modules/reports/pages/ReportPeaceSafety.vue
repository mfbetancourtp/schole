<template>
  ...
  <BaseReport v-else :title="title" :v$="v$" :routes="routes" :service="service" :params="params" :show-btn-generate="false">
    ...
    <div class="responsive-buttons">
      ...
      <AppButton :label="'Publicar todos'" :disabled="!showListStudents" class="buttons" :native-type="'button'" @click="publicAll()" variant="primary"></AppButton>

      <AppButton :label="'reports.filters.sendGenerated'" :disabled="!showListStudents" class="buttons" :native-type="'button'" @click="sendMailAll()" variant="primary"></AppButton>
...
const generatePeaceSafetyPerStudentService = new GeneratePeaceSafetyPerStudentService();
...
export default defineComponent({
  ...
  setup() {
    ...
    const generateIndividualReport = async (data: any) => {
      ...
      try {
        const response = await generatePeaceSafetyPerStudentService.run({
          ...getFilterReports(),
          userId: data.userId,
