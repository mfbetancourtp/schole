<template>
  <AppBaseList :title="t('Tipos de Programas')">
    <template #actions>
    ...
    <template v-slot:content>
      <AppDatatable :service="service" name="programTypes" :tableTitle="t('Tipos de Programas Académicos')" forceShowFilter :showFilterSteps="false">
        ...
        <AppDatatableColumn :label="t('core.table.actions')" v-slot="{ row }" :position="'centered'" width="9rem">
          <div class="d-flex justify-content-center">
            <AppButtonGroup size="sm">
              ...
              <AppButton icon="sitemap" outlined variant="info" v-tooltip="t('Estructura')" size="sm" disabled> </AppButton>
      ...
      <AppModal v-model="isModalVisible">
        <ProgramTypeFormModal v-if="isModalVisible" :item="selectedItem" @close="isModalVisible = false" @save="handleSave" />
      </AppModal>

      <!-- Modal de Confirmación de Eliminación -->
      <AppModal v-model="modalDelete">
        <AppConfirmDeleteModal v-if="modalDelete" :entity="t('Tipo de Programa')" @confirmDelete="confirmDelete" @close="modalDelete = false"></AppConfirmDeleteModal>
      </AppModal>
    </template>
  </AppBaseList>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleSave = async (form: ProgramTypeDto) => {
      const id = selectedItem.value ? selectedItem.value.id : undefined;
